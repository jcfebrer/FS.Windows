...
{
    public class DBButtonCollectionEx : CollectionBase, IBindingList
    {
        private ListChangedEventHandler onListChanged;

        public DBButtonEx this[int index]
        {
            get { return (DBButtonEx) List[index]; }
            set { List[index] = value; }
        }

        public DBButtonEx this[string key] => get_Find(key);
        ...
        public DBButtonEx get_Find(string key)
        {
            foreach (DBButtonEx dbbutton in List)
        ...
        public int get_GetColumnOrdinal(string name)
        {
            ...
            foreach (DBButtonEx dbcol in List)
            {
        ...
        public void AddRange(DBButtonEx[] Values)
        {
            var f = 0;
        ...
        public void Remove(DBButtonEx Value)
        {
            List.Remove(Value);
        }


        public void Insert(int index, DBButtonEx Value)
        {
            List.Insert(index, Value);
        }


        public bool Contains(DBButtonEx Value)
        {
            return List.Contains(Value);
        }


        public int IndexOf(DBButtonEx Value)
        {
